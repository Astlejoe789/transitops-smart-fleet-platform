...
export function SystemHealthCard() {
  ...
  return (
    <div className="flex flex-col justify-between rounded-[20px] bg-[#0B1426]/80 border border-surface-800/40 p-5 min-h-[150px]">
      <div>
        <h3 className="text-[15px] font-bold text-white mb-4 text-center" style={{ fontFamily: 'Outfit, Inter, sans-serif' }}>
          System Health
        </h3>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          {services.map((item) => (
            <div key={item.name} className="flex flex-col items-center gap-1 text-center">
              ...
              <span className="text-[10px] text-surface-400">{item.status}</span>
            </div>
          ))}
        </div>
      </div>

      <div className="flex items-center justify-center gap-2 pt-3 border-t border-surface-800/40 text-[11px] font-medium text-success">
